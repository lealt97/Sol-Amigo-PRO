...
- [x] Segurança e recuperação de conta
...
Evidência de RLS: as 18 tabelas do schema `public` foram verificadas com RLS ativo. A auditoria automatizada exige cobertura CRUD nas tabelas pertencentes ao usuário, vínculo das políticas privadas com `auth.uid()`, eventos de proposta e cobrança append-only, catálogos globais somente leitura e ausência de views públicas não auditadas. `proposal_sequences` e `mfa_recovery_codes` permanecem sem acesso direto de `anon` ou `authenticated`; `mfa_security_events`, `subscriptions`, `billing_events` e `account_usage` permitem somente leitura dos registros da própria conta, enquanto `billing_plans` expõe apenas os planos públicos e ativos. Funções `SECURITY DEFINER` usam `search_path` fixo; funções de gatilho não podem ser chamadas diretamente pela API; RPCs de conta exigem autenticação; e as RPCs públicas por token permanecem disponíveis para visualização, aprovação e recusa. Nenhuma regra funcional, capa, cálculo ou tela comercial foi alterada.
...
Evidência de backup e restauração: a homologação criou uma fixture determinística em 20 tabelas cobrindo usuário, identidade, MFA, hashes de códigos de recuperação, eventos de segurança MFA, assinatura, eventos de cobrança, uso mensal, perfil, cliente, kit, proposta, cálculos, cargas, eventos, modelos de PDF, sequência e metadados do Storage. Um arquivo lógico em formato custom foi gerado pelo PostgreSQL 17, identificado por SHA-256 e manifesto; em seguida, os registros da fixture e todos os metadados do Storage foram removidos para simular perda. A restauração foi executada com gatilhos ativos e conexões separadas para `supabase_auth_admin`, `postgres` e `supabase_storage_admin`. Os fingerprints anteriores e posteriores foram idênticos, todos os vínculos relacionais foram confirmados, incluindo usuário → assinatura → evento → uso, e a fixture foi eliminada no final. O teste ocorre somente no Supabase local descartável e não copia nem modifica dados reais. O backup PostgreSQL cobre buckets e registros de objetos, mas não os bytes armazenados fora do banco; essa parte é tratada no procedimento de recuperação de desastre.
...
- [x] Definir processo administrativo seguro de recuperação de conta
...
Evidência do processo administrativo seguro de recuperação de conta: `docs/ADMIN_ACCOUNT_RECOVERY_PROCESS.md` define triagem, estados append-only, verificação por sinais independentes, tratamento especial para perda do e-mail original, períodos de espera de 24 ou 48 horas, notificações, segregação entre atendente, verificador, aprovador e executor, autenticação administrativa em AAL2 com senha recente, duplo controle, revogação global de sessões, invalidação dos códigos antigos, remoção de fatores exclusivamente pela Auth Admin API e nova configuração obrigatória do MFA. O documento proíbe senha temporária, coleta de segredos, SQL improvisado, edição manual de tabelas e `service_role` no frontend. A execução permanece bloqueada enquanto papel administrativo validado no servidor, ferramenta interna, canal seguro, notificações, política LGPD e auditoria não estiverem disponíveis; essas implementações continuam na Fase 4. Testes de regressão verificam as salvaguardas e a integração com o procedimento de perda do celular, README e checklist.

## Fase 3 — Planos, cobrança e assinatura

- [x] Definir plano gratuito, mensal e anual
- [x] Definir limites de propostas, usuários e armazenamento
- [x] Criar tabelas de planos, assinaturas, eventos e uso
...
Evidência da definição comercial: o catálogo versionado possui apenas os produtos `free` e `pro`. O Gratuito custa R$ 0,00 e não exige cartão; o Pro custa R$ 100,00 por mês ou R$ 1.000,00 por ano, com cobrança anual antecipada equivalente a dez mensalidades. Mensal e anual são intervalos do mesmo produto e compartilham funcionalidades. Valores ficam em centavos inteiros, IDs externos permanecem no servidor e nenhum preço recebido do navegador é considerado fonte de verdade.

Evidência dos limites: o Gratuito possui 5 propostas por mês, 1 usuário e 250 MiB; o Pro possui 100 propostas por mês, 5 usuários e 10 GiB. A janela de propostas usa o mês civil em `America/Sao_Paulo`; criação e duplicação consomem cota, enquanto edição e regeneração de PDF não consomem outra unidade. Avisos começam em 80%, dados nunca são removidos automaticamente e o downgrade preserva todo o conteúdo existente, bloqueando somente novas operações acima da cota.

Evidência da fundação de cobrança: a migration cria `billing_plans`, `subscriptions`, `billing_events` e `account_usage`, todas com RLS. Contas novas e existentes recebem de forma idempotente uma assinatura Gratuita, um período mensal de uso e o evento `subscription.initialized`. O frontend pode consultar catálogo público e somente os registros da própria conta, sem permissão direta de escrita. Eventos possuem chave externa única parcial para deduplicação futura, funções internas têm `search_path` fixo e não são executáveis pela API. A homologação com duas identidades confirmou isolamento, imutabilidade dos eventos, privilégios e inclusão de assinatura, eventos e uso no backup/restauração.
