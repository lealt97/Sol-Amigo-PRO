-- Migration: modelos PDF do usuário no Supabase
-- Execute este arquivo no SQL Editor do Supabase caso ainda não use migrations automatizadas.

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Garante profiles para usuários existentes antes de adicionar FK direta proposals -> profiles.
...
-- Permite o relacionamento PostgREST: proposals.user_id -> profiles.id
-- Necessário para selects como profile:profiles(...).
DO $$
BEGIN
...
-- Tabela atual do novo Design PDF.
-- O id e o preset_id são TEXT para compatibilizar com IDs do front-end, como preset-1.
